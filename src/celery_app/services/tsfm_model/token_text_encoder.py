...
class TokenTextEncoder(nn.Module):
    ...
    def _init_model(self):
        ...
        # Freeze and move to CPU explicitly (avoid MPS issues)
        for p in auto_model.parameters():
            p.requires_grad = False
        auto_model = auto_model.to('cpu')

        # Store without registering as submodule (keeps out of state_dict)
        object.__setattr__(self, '_model', auto_model)
    ...
    def encode(
        ...
        # Force CPU device to avoid MPS issues with some transformer models
        if device.type == 'mps':
        ...
        if uncached_texts:
            ...
            # Cache newly encoded texts (keep on device to avoid transfers every batch)
